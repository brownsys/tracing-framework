...
public class Delete implements Benchmark {

    @Override
    public void run(FileSystem hdfs, int pid, int wid, HDFSAggregator aggregator, String... opts) {
        String prefix = String.format("/retro/benchmarks/delete/pid%d/worker%d", pid, wid);
        ...
        while (!Thread.currentThread().isInterrupted()) {
            ...
            try {
                ...
                if (!FSUtils.deleteFile(hdfs, file))
                    throw new IOException("Unable to delete " + file);
                long duration = System.nanoTime() - begin;
                aggregator.startedAndFinished(Operation.DELETE, 1, 1, duration);
            } catch (IOException e) {
